                ...
                if st.button("🔍 Generate LIME Explanation", type="primary"):
                    with st.spinner("Generating LIME explanation... This may take 30-60 seconds."):
                        try:
                            ...
                            st.write("- 🟢 **Green boundaries**: Image regions that strongly support the AI's prediction")
                            st.write("- 🔍 **Highlighted areas**: Most influential features the AI used for diagnosis")
                            st.write("- 📊 **LIME Algorithm**: Tests model by systematically hiding image regions")
                            st.write("- 🏥 **Medical Insight**: These highlighted areas should align with known diagnostic features")
...
def advanced_bias_detection_page():
    ...
    if len(stats) < 10:
        st.warning(f"⚠️ Need at least 10 predictions for advanced bias analysis. Current count: {len(stats)}")
        st.info("💡 Make more predictions to enable Threshold Optimization post-processing.")
        return
    ...
    if not bias_detector.is_fitted and len(stats) >= 20:
        with st.spinner("Training Threshold Optimizer for equalized odds..."):
            ...
            if success:
                ...
                st.warning("⚠️ Could not train post-processor. Using standard analysis.")
    ...
    with st.expander("📚 About Threshold Optimization for Equalized Odds"):
        st.write("""
        ...
        **Benefits:**
        ...
        - ✅ Preserves overall model accuracy when possible
        - ✅ Provides mathematical fairness guarantees
        - ✅ Can be applied without model retraining
        - ✅ Interpretable and explainable adjustments
        
        **Limitations:**
        - ⚠️ Requires labeled data with sensitive attributes
        - ⚠️ May reduce overall accuracy for fairness
        - ⚠️ Assumes binary sensitive attributes and outcomes
        """)
    ...
    if y_pred_fair is not None:
        recommendations.append("✅ **Post-processing Active**: Fairness-adjusted predictions are being generated")
    else:
        recommendations.append("📈 **Collect More Data**: Need more predictions to enable advanced fairness techniques")
    
    if not recommendations:
        recommendations.append("✅ **Good Fairness**: No major bias concerns detected with current data")
...
if stats:
    ...
    with col1:
        ...
        st.write("**Confidence Distribution by Prediction**")
        fig = px.box(df, x='prediction', y='confidence', 
                    title="Confidence Levels by Prediction Type",
                    color='prediction')
        fig.update_layout(
            height=400,
            plot_bgcolor='white',
        ...
        benign_conf = df[df['prediction'] == 'benign']['confidence'].mean()
        malignant_conf = df[df['prediction'] == 'malignant']['confidence'].mean()
        conf_diff = abs(benign_conf - malignant_conf)
        
        if conf_diff > 0.1:
            st.warning(f"⚠️ **Confidence Bias Detected**: {conf_diff:.1%} difference in average confidence")
        else:
            st.success("✅ **No significant confidence bias detected**")
    
    with col2:
        # Prediction trends over time
        st.write("**Prediction Trends Over Time**")
        
        # Add hour to timestamp for better granularity
        df['date_hour'] = df['timestamp'].dt.floor('H')
        df_time = df.groupby(['date_hour', 'prediction']).size().reset_index(name='count')
        
        if len(df_time) > 0:
            fig = px.line(df_time, x='date_hour', y='count', color='prediction',
            ...
            st.info("Not enough data points for time series visualization")
            
            # Show simple count instead
            pred_counts = df['prediction'].value_counts()
            fig_alt = px.bar(x=pred_counts.index, y=pred_counts.values,
                            title="Total Prediction Counts",
    ...
    with col1:
        ...
        benign_rate = len(df[df['prediction'] == 'benign']) / len(df)
        malignant_rate = len(df[df['prediction'] == 'malignant']) / len(df)
        
        st.metric("Benign Prediction Rate", f"{benign_rate:.1%}")
        st.metric("Malignant Prediction Rate", f"{malignant_rate:.1%}")
        
        if abs(benign_rate - 0.5) > 0.3:
            st.warning("⚠️ Potential class imbalance in predictions")
        else:
            st.success("✅ Balanced prediction distribution")
    
    with col2:
        # Confidence calibration
        high_conf_correct = len(df[df['confidence'] > 0.8])
        low_conf_total = len(df[df['confidence'] < 0.6])
        
        st.metric("High Confidence Predictions", high_conf_correct)
        st.metric("Low Confidence Predictions", low_conf_total)
        
        if low_conf_total > len(df) * 0.2:
            st.warning("⚠️ High rate of low-confidence predictions")
        else:
            st.success("✅ Good confidence calibration")
    
    with col3:
        # Temporal consistency
        recent_preds = df.tail(10)['prediction'].value_counts()
        
        st.write("**Recent 10 Predictions:**")
        for pred, count in recent_preds.items():
            st.write(f"{pred.title()}: {count}")
        
        if len(recent_preds) == 1:
            st.warning("⚠️ Only one prediction type recently")
        else:
            st.success("✅ Diverse recent predictions")
    ...
    if conf_diff > 0.1:
        recommendations.append("🔧 **Confidence Calibration**: Consider retraining with balanced confidence targets")
    
    if low_conf_total > len(df) * 0.2:
        recommendations.append("� **Model Improvement**: High uncertainty suggests need for more training data")
    
    if abs(benign_rate - 0.5) > 0.3:
        recommendations.append("⚖️ **Class Balance**: Monitor for potential bias toward one diagnosis type")
    
    if not recommendations:
        recommendations.append("✅ **Good Performance**: No major bias concerns detected")
...
stats = blockchain.get_prediction_stats()
if stats:
    st.sidebar.metric("Total Predictions", len(stats))
    benign_count = len([s for s in stats if s['prediction'] == 'benign'])
    malignant_count = len([s for s in stats if s['prediction'] == 'malignant'])
    if benign_count > 0:
        st.sidebar.write(f"🟢 Benign: {benign_count}")
